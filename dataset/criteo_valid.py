...
class CriteoDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_path=None, cache_path='.criteo', rebuild_cache=False, min_threshold=10):
        ...
        self.prefix = "D:/work/My_model_dataset/criteo/"
    ...
    def __build_cache(self, path, cache_path):
        temp_path = self.prefix + "train_100000.txt"
        # count feature map
...
def get_criteo_dataset_loader_valid(train_path="train_100000.txt", test_path="test_10000.txt", batch_size=256):
    # the test_path maybe null, if it is, we need to split the train dataset
    print("start load dataset from valid ache")
    prefix = "D:/work/My_model_dataset/criteo/"
    prefix_c = "D:/work/My_model/frnet/criteo"
    ...
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=batch_size, shuffle=True, num_workers=0,drop_last=True,pin_memory=True)

    valid_loader = torch.utils.data.DataLoader(
        valid_dataset, batch_size=batch_size, shuffle=False, num_workers=0,drop_last=False,pin_memory=True)

    test_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=batch_size, shuffle=False, num_workers=0,drop_last=False,pin_memory=True)
